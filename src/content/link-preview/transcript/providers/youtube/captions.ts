...
export const fetchTranscriptFromCaptionTracks = async (
  fetchImpl: typeof fetch,
  { html, originalUrl, videoId, skipAutoGenerated }: YoutubeTranscriptContext & { skipAutoGenerated?: boolean }
): Promise<string | null> => {
  ...
  try {
    ...
    // Don't fall back to android player if we're in manual-only mode (it may return auto captions)
...
const extractTranscriptFromPlayerPayload = async (
  ...
  const filteredTracks = skipAutoGenerated
  ...
  return await findFirstTranscript(fetchImpl, filteredTracks, 0)
}
